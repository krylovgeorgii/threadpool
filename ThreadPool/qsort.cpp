...
template<class Iter>
Iter partQSort(Iter begin, Iter end, int64_t dis) {	
	--end;

	auto pivot = [dis, begin, end] {
		auto middle = *(begin + dis / 2);

		if (*begin <= middle && middle <= *end || *begin == *end) {
			return middle;
		}

		if (middle <= *begin && *begin <= *end) {
			return *begin;
		}
